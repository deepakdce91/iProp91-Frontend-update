...
function Home() {
  ...
  const updateFiltersWithLocation = async (coords) => {
    try {
      ...
      const city = data.address?.city || data.address?.town || data.address?.village || 'Any';
  ...
  const fetchPropertiesInBounds = async (retryCount = 0) => {
    ...
    fetchingRef.current = true;
    setIsLoading(true);
    console.log(`Starting fetch attempt ${retryCount + 1} with filters`, filters);
  ...
  const processApiResponse = (fetchedProperties, retryCount = 0) => {
    ...
    if (fetchedProperties && fetchedProperties.data && 
       (fetchedProperties.data.projects || fetchedProperties.data.properties)) {
      
      // Handle both API response formats
      const projectsArray = fetchedProperties.data.projects || fetchedProperties.data.properties || [];
      ...
      if (Array.isArray(projectsArray)) {
        ...
        localStorage.setItem('cachedProperties', JSON.stringify(processedProperties));
  ...
  const loadFallbackData = () => {
    ...
    localStorage.setItem('cachedProperties', JSON.stringify(fallbackProperties));
  };
  ...
  return (
    <>
        ...
        <section className="w-3/5 max-w-[850px] p-5 overflow-y-auto h-[calc(100vh-60px-77px)]">
          <div className="flex justify-between items-center mb-5 flex-wrap">
            <h2 className="text-[22px] font-semibold">
              {isLoading ? "Finding properties..." : `Over ${properties.length} places within `}
              {userLocation ? "your area" : locationDenied ? "available properties" : "selected area"}
              {filters.categoryFilter !== 'all' ? ` - ${filters.categoryFilter} category` : ''}
            </h2>
      ...
      {showFilters && (
        <FiltersModal allFilters={filters} setFilters={setFilters} closeModal={() => setShowFilters(false)} />
      )}
    </>
  );
}
