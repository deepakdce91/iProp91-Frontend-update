...
export default function SearchBar({
  ...
  // Expose getCurrentLocation via window for the map component to use
  useEffect(() => {
    ...
    return () => {
      ...
      window.updateSearchbarLocation = undefined;
    };
  }, []);
  ...
  const fetchLocationFromCoords = async (latitude, longitude) => {
    try {
      // First attempt with BigDataCloud API
      const response = await axios.get(
        `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${latitude}&longitude=${longitude}&localityLanguage=en`
      );

      if (response.data) {
        const { city, principalSubdivision, locality } = response.data;
        console.log("Location data:", response.data);
        
        // Store detected location in window object for map component
        ...
        if (matchedState) {
          ...
          // Try to find the city in our cities list
          setTimeout(async () => {
            // Find matching city in our cities list
            let matchedCity = null;
            
            // Try with city first, then with locality if no match
            const cityName = city || locality;
            ...
            const locationFilters = {
              ...
              // Keep other filter values that might already be set
              propertyType: [],
              bhk: [],
              minBudget: "",
              maxBudget: "",
            ...
            // Update city selection in UI if we found a match
            if (matchedCity) {
              setSelectedCity(matchedCity);
            }
            
            // Always apply the filters, even if city is empty
            onFilterChange(locationFilters);
            
          }, 1000); // Give time for cities to load
        } else {
          // If no state match, still try to use the location data from the API
          console.log("No matching state found for:", principalSubdivision);
          
          // Just use the raw location data from the API
          const locationFilters = {
            state: principalSubdivision || '',
            city: city || locality || '',
            propertyType: [],
          ...
          console.log("Applying raw location filters:", locationFilters);
          onFilterChange(locationFilters);
        }
      }
    } catch (error) {
      ...
      try {
        ...
        if (fallbackResponse.data && fallbackResponse.data.address) {
          const { state, city, town, village } = fallbackResponse.data.address;
          const detectedCity = city || town || village || '';
          const detectedState = state || '';
          
          console.log("Fallback location data:", fallbackResponse.data.address);
          
          // Store detected location in window object
          window.userLocationData = {
            latitude,
            longitude,
            city: detectedCity,
          ...
          console.log("Applying fallback location filters:", locationFilters);
          onFilterChange(locationFilters);
        }
      } catch (fallbackError) {
  ...
  return (
    <div className="bg-gradient-to-r from-gray-900 to-black shadow-lg w-full">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex-1 w-full flex flex-col md:flex-row gap-4">
            ...
            <button
              ...
              disabled={isLocating}
              className="w-full md:w-auto px-4 py-3 rounded-xl border-2 border-gold-400 text-white font-medium flex items-center justify-center gap-2 hover:bg-gold-400/10 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <FaLocationArrow
            ...
            {/* State Selection */}
            <div className="relative flex-1 w-full" ref={stateDropdownRef}>
